...
# -------------------------------------------------------------------------
# CONEXÃO COM O BANCO DE DADOS (PostgreSQL - persistente no Render)
# -------------------------------------------------------------------------
def get_conn():
    database_url = os.environ.get("DATABASE_URL")
    if not database_url:
        st.error("Variável DATABASE_URL não encontrada. Configure-a nas Environment Variables do Render.")
        st.stop()
...
def init_db():
    ...
    # Clientes
    cursor.execute("CREATE TABLE IF NOT EXISTS clientes (id SERIAL PRIMARY KEY, nome TEXT NOT NULL, cnpj TEXT)")

    # Funcionários
    cursor.execute("CREATE TABLE IF NOT EXISTS funcionarios (id SERIAL PRIMARY KEY, nome TEXT NOT NULL, cargo TEXT)")

    # Fluxo de Caixa com suporte a detalhes de empréstimo externo
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS fluxo_caixa (
            ...
            destino_saude REAL,
            origem_conta TEXT,
            credor_emprestimo TEXT,
            juros_emprestimo REAL,
    ...
    cursor.execute("""
        SELECT column_name FROM information_schema.columns WHERE table_name = 'fluxo_caixa'
    """)
...
def atualizar_saldos_e_config(regime, simples, pis, cofins, iss, irpj, csll, comissao, s_normal, s_saude):
    ...
    cursor.execute("""
        ...
        regime_tributario=%s, aliquota_simples=%s, pis=%s, cofins=%s, iss=%s, irpj=%s, csll=%s, comissao_padrao=%s,
            ...
            if st.form_submit_button("Confirmar Entrada de Empréstimo"):
                if credor:
                    ...
                    inserir_movimentacao(
                        ...
                        cliente_id=None,
                        func_id=None,
                        desc=f"Empréstimo recebido de {credor}",
                        v_bruto=v_emp,
                        v_imp=0.0,
                        v_com=0.0,
                        v_liq=v_emp,
                        nf="SEM NF (EMPRÉSTIMO)",
...
# -------------------------------------------------------------------------
    ...
    with st.form("form_transferencia"):
        ...
        if st.form_submit_button("Executar Resgate para Conta Normal"):
            if v_transf > 0:
                inserir_movimentacao(
                    tipo='Transferência', categoria='Resgate Emergencial', cliente_id=None, func_id=None,
                    desc=f"Resgate Saúde -> Conta Normal: {motivo_transf}", v_bruto=v_transf, v_imp=0.0, v_com=0.0, v_liq=v_transf,
                    nf="", dt_ven=datetime.now().strftime('%Y-%m-%d'), dt_pag=datetime.now().strftime('%Y-%m-%d'), status='Pago/Recebido', d_saude=0.0, origem='Saúde da Empresa'
                )
